...
import { createGetNextBusinessDay } from './index.js';
import { toJstDate } from '../_internal/jst.js';

describe('createGetNextBusinessDay', () => {
  // 2025-01-01 元日、2025-01-13 成人の日
  const mockHolidayDates = new Set(['2025-01-01', '2025-01-13']);
  const getNextBusinessDay = createGetNextBusinessDay(mockHolidayDates);
  ...
  describe('Map をデータソースとして使用', () => {
    it('Map でも動作する', () => {
      ...
      const getNextBusinessDayWithMap = createGetNextBusinessDay(holidayNames);
      const result = getNextBusinessDayWithMap('2025-01-06');
      assert.strictEqual(result.getTime(), toJstDate('2025-01-07').getTime());
    });
  });
});
