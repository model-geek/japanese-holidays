...
このプロジェクトはライブラリであり、アプリケーション開発とは異なる基準が求められる。
...
- **利用者の規模を制御できない**: ライブラリは誰がどのような規模で使うか分からない。アプリケーション開発では「このデータは最大でも数百件」と分かっていても、ライブラリでは数万件で呼ばれる可能性がある
- **呼び出し頻度を制御できない**: アプリケーションでは「この処理は 1 回しか呼ばれない」と分かっていても、ライブラリではホットパスで何度も呼ばれる可能性がある
- **環境を制御できない**: ブラウザ・Node.js・エッジランタイムなど、様々な環境のスタック制限やメモリ制約に対応する必要がある
- **性能劣化が利用者全体に波及する**: ライブラリの非効率なコードは、それを使う全てのアプリケーションに影響する
