...
- 要素列・配列・木構造などの走査は、ループで実装すること
- 入力サイズや深さに外部からの制約がないライブラリコードでは、再帰を原則禁止とする

**理由**

- JavaScript エンジンには末尾再帰最適化(TCO)の保証がなく、再帰のネストが深くなると **スタックオーバーフロー (`RangeError`) のリスク** があります
- ライブラリコードは呼び出し側の入力サイズ・深さを制御できず、様々な環境(ブラウザ・Node.js)のスタック制限に依存するため、安全性の観点から再帰を避けてください

### 2-2. 再帰を使ってよいケース

再帰を使用してよいのは、次の条件を **すべて** 満たす場合のみです:

- 再帰の深さに **理論的な上限があり**、それがスタック的に十分浅いと判断できる
- その上限や前提を、コメントでコード上にはっきり明示している
- ユニットテスト等で、その範囲内で正常に動作することを確認している

上記条件を満たさない一般的な走査・探索処理は、ループ + 明示的スタック(配列)で実装してください。
