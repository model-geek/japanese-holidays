...
## 3. アプリケーションコードとライブラリコードの違い

### 3-1. アプリケーションコード

**規約**

- アプリケーション固有のドメインロジックでは、
  - データサイズ・構造・性能要件がチーム内で把握できている範囲で、
  - `const` + 再生成(`[...a, x]`, `{...obj, foo: 1}`)や、
    条件付きでの再帰などを **可読性重視で採用してよい**
- ただし、明らかにボトルネックとなる箇所では、
  - ライブラリコードと同様の基準(ミュータブル + ループ)に切り替えること

**理由**

- アプリケーション側は、
  - 具体的なユースケースやデータ規模が分かっており、
  - 「多少遅くても、読みやすさ・変更しやすさを優先」が許されやすい
- それでもボトルネックとなる箇所は存在し得るため、
  プロファイル結果などに応じて方針を切り替えられるようにしておく

### 3-2. ライブラリ・共通ユーティリティコード

**規約**

- 多数の場所から利用されるライブラリ/ユーティリティでは、
  - 本ドキュメントの
    - 「外部 API はイミュータブル」
    - 「内部は性能のためにミュータブルも許容」
...
- 性能クリティカルな箇所(大規模データや高頻度呼び出しが想定される処理)は、
  - コード中にその旨コメントを残し、
  - 必要に応じてベンチマークやプロファイル結果を添付すること

**理由**

- ライブラリは利用者・用途を制限できないため、
  - スケールしたときの安全性・性能を優先する必要がある
- 理由をコメントで残しておくことで、
  - 後から読む人が「なぜミュータブルで書いているのか」を理解しやすくなり、
  - 不用意な「きれい化リファクタリング」による性能劣化を防げる
